...
m00 = m00.applymap(lambda x: x.strip() if isinstance(x, str) else x)

with st.sidebar:
    st.title('INFORACTIVE-손익예측')
    #st.markdown('### Material Control')
    
    expander1 = st.expander("🔢 Parameter Controls", expanded=True)
    option = expander1.selectbox(
    'Select products', m00.loc[(m00["kind"]=="FERT")])
    ...
    'Select raw materials', m00.loc[(m00["kind"]=="ROH")])
    #m50change1 = expander1.slider('원재료 구매환율 변화%', -12.0, 12.0, 0.0, 0.1)/100
...
with col[1]:
    ...
    tabs = st.tabs(["수불부", "원가계산서", "손익요약", "제조원가명세서-요약", "제조원가명세서"])
...
def send_request():
    ...
    if not user_msg:
        return
    st.session_state.messages.append({"role": "user", "content": user_msg}) # 채팅창에 사용자 쿼리 올리기
    ...
    response = requests.post(FASTAPI_ENDPOINT, data=payload)

    
    if response.status_code == 200:            
        result = response.json().get("response", "")
        output= result.get("output", "")
        reference_urls= result.get("reference_url", "')")
...
with col[2]:
    ...
    option3=st.selectbox('haha',
     m00.loc[m00["kind"]=="FERT"],label_visibility="hidden",index=6)
