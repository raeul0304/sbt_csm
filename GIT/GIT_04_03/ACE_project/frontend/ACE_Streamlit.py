...
from chatbot_ui import render_chatbot, initialize_session_state
...
col = st.columns((1.1, 4, 2), gap='large')
...
m00=pd.read_csv('../data/m00_csv.csv')
m00=m00[['mat','kind']]
m93show=pd.read_csv('../data/m93show_csv.csv')
m93show=m93show.iloc[:,1:]
mm=pd.read_csv('../data/mm_csv.csv')
mm=mm.set_index('mat')
...
with col[2]:
    ...
    option3 = st.selectbox('제품 선택', fert_options, label_visibility="hidden", index=default_option3_idx, key="option3_selectbox")
    ...
    fig1 = px.pie(mpie, values=option3,names='index',color_discrete_sequence=['#1f77b4', '#5dc2d6', '#79d9c3', '#a2e8dd', '#88cc88', '#6dbb6d', '#4da64d'])#names='country',
